...
{
	for (KX_Material *mat : converter.m_materials) {
...
bool BL_Converter::FreeBlendFileData(Main *maggie)
{
	...
	for (auto& pair : m_sceneSlots) {
		...
		// Free materials.
		for (UniquePtrList<KX_Material>::iterator it = sceneSlot.m_materials.begin(); it != sceneSlot.m_materials.end(); ) {
			KX_Material *mat = it->get();
...
{
	for (std::unique_ptr<KX_Material>& mat : m_sceneSlots[scene].m_materials) {
		mat->ReloadMaterial();
	}
